...
## Step 4 — 💳 Generate and send the PayNow link

UW generates the PayNow link directly in NMI — it is **not** requested from Management.

> 📖 Full steps (NMI login, Collect Checkout, Button Generator, product setup, and the Secure Payment Link email) are documented in **[PayNow Link](./bds-paynow-link)**. Follow that page start to finish, then come back here to continue with Step 5.

Quick summary:

1. Log in to NMI and generate the payment link for this client (see [PayNow Link](./bds-paynow-link) for the full steps).
...
When a contract was sent with incomplete documentation (Next Step: "Missing documents to enroll" — see [02 — Contract Request](./bds-02-contract-request)) and the missing documents arrive:
...
- The PayNow link is generated by UW directly in NMI — see [PayNow Link](./bds-paynow-link). It is not requested from Management.
...
- Whenever a document is received (contract, bank statement, license), verify it and extract the relevant details, updating the merchant's name in Salesforce to match the contract if needed — see [05 — Quick Reference, Section 6](./bds-05-quick-reference#section-6--document-verification).
...
- [ ] PayNow link generated in NMI (see [PayNow Link](./bds-paynow-link)) and pasted into Card Payment Info → PayNow Link

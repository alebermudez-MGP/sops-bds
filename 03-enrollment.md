...
> 📖 Full steps (NMI login, Collect Checkout, Button Generator, product setup, and the Secure Payment Link email) are documented in **[PayNow Link](./paynow-link)**. Follow that page start to finish, then come back here to continue with Step 5.

Quick summary:

1. Log in to NMI and generate the payment link for this client (see [PayNow Link](./paynow-link) for the full steps).
...
- The PayNow link is generated by UW directly in NMI — see [PayNow Link](./paynow-link). It is not requested from Management.
...
- [ ] PayNow link generated in NMI (see [PayNow Link](./paynow-link)) and pasted into Card Payment Info → PayNow Link
